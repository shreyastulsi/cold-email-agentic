...
import KeryxLogo from './keryx.svg';
import KeryxIcon from './keryx_icon.svg';

export const Logo = ({ className, iconOnly }: { className?: string; iconOnly?: boolean; uniColor?: boolean }) => {
    return (
        <img 
            src={iconOnly ? KeryxIcon : KeryxLogo} 
            alt="Keryx Logo" 
            className={cn(iconOnly ? 'h-8 w-8' : 'h-12 w-auto', className)}
        />
    )
}
